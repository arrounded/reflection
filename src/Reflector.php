...
use Arrounded\Database\Repositories\AbstractRepository;
...
class Reflector
{
    ...
    public function getNamespaceFolder($namespaces, $folder = null)
    {
        ...
        foreach ($namespaces as $namespace) {
            $folders[] = $this->namespaceToFolder($folder);
            $folders[] = $this->namespaceToFolder($namespace.'\\'.$folder);

            // Account for PSR4
            $namespace = str_replace($this->namespace, null, $namespace);
            $folders[] = $this->namespaceToFolder($namespace.'\\'.$folder);
        }
    ...
    /**
     * Translate a namespace to a folder
     *
     * @param string $namespace
     *
     * @return string
     */
    public function namespaceToFolder($namespace)
    {
        $folder = trim($namespace, '\\');
